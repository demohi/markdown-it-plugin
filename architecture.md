...
输入的数据通过三个步骤被解析，分别是core，block以及 inline
...
被解析的结果是*一系列*的 tokens,这些数据会被传递到`renderer`，然后会被生成 html。
...
这些 tokens 可以被再次解析成更多的 tokens（例如一个`list token` 可以被分为更多的 `inline tokens`）;

An `env` sandbox can be used alongside tokens to inject external variables for your parsers and renderers.

一个`env` 沙盒 可以被`tokens`用作为一些parsers 和renders 注入额外的变量。
...
当编译数据时，每一个步骤（core、block、inline） 都使用一个独立的`state` 对象，这样每个编译函数都是独立的，可以被单独取消。
